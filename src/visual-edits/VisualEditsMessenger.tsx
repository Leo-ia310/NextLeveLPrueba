...
export const CHANNEL = "NEXTLEVEL_HOVER_v1" as const; 
const VISUAL_EDIT_MODE_KEY = "nextlevel_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "nextlevel_focused_element" as const;

// Deduplicate helper for high-frequency traffic (HIT / FOCUS_MOVED / SCROLL)
// -----------------------------------------------------------------------------
let _nextlevelLastMsg = "";
const postMessageDedup = (data: any) => {
  try {
    const key = JSON.stringify(data);
    if (key === _nextlevelLastMsg) return; // identical – drop
    _nextlevelLastMsg = key;
  } catch {
...
// Helper to parse data-nextlevel-id to extract file path, line, and column
const parseNextlevelId = ( 
  nextlevelId: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = nextlevelId.split(":");
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-nextlevel-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-nextlevel-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the nextlevel ID from the element to ensure we're working with the right one
    const nextlevelId = element.getAttribute("data-nextlevel-id");
    ...
    if (newText !== oldText) {
      const parsed = parseNextlevelId(nextlevelId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same nextlevel ID
    const allMatchingElements = document.querySelectorAll(
      `[data-nextlevel-id="${nextlevelId}"]`
    ...
    const existingStyles = appliedStylesRef.current.get(nextlevelId) || {};
    appliedStylesRef.current.set(nextlevelId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(nextlevelId);
  ...
  const flushImageSrcChange = () => {
    ...
    const nextlevelId = imgElement.getAttribute("data-nextlevel-id");
    if (!nextlevelId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "nextlevel_STYLE_UPDATE") {
        const { elementId, styles } = e.data;

        // Find ALL elements with the same nextlevel ID
        const allMatchingElements = document.querySelectorAll(
          `[data-nextlevel-id="${elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === "nextlevel_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-nextlevel-id
        const nextlevelId = element.getAttribute("data-nextlevel-id");
        if (nextlevelId) {
          const parsed = parseNextlevelId(nextlevelId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-nextlevel-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-nextlevel-id");
        ...
        const tagName =
          hit.getAttribute("data-nextlevel-name") || hit.tagName.toLowerCase();

        // Update hover boxes immediately for instant feedback
        // Find ALL elements with the same nextlevel ID
        const allMatchingElements = document.querySelectorAll(
          `[data-nextlevel-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-nextlevel-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-nextlevel-id]");
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same nextlevel ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-nextlevel-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        // Clicked on empty space or element without data-nextlevel-id
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        // Find ALL elements with the same nextlevel ID
        const allMatchingElements = document.querySelectorAll(
          `[data-nextlevel-id="${e.data.elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-nextlevel-name") ||
